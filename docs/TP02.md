...
```shell
kubectl create deployment hello-world --image=stefanprodan/podinfo:6.11.2
```
...
Depuis le terminal CodeSpaces (ou une station Linux), verifier que le load-balancer dispatche bien les requêtes, par exemple en lançant plusieurs curl :
```shell
for i in {1..100}; do 
    curl -s http://<IP_PUBLIC> | jq '.hostname'  ; done | sort  | uniq -c
```
...
```shell
kubectl set image deployment/hello-world podinfo=stefanprodan/podinfo:5.2.0
```
