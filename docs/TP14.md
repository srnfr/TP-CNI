...
Installons le CNI
```shell
helm upgrade --install --namespace kube-system --repo https://helm.cilium.io cilium cilium --version 1.16.1 --values ebpf-values-nokubeproxy.yaml
```

## Hubble CLI

A l'instar de `cilium`, il existe un CLI pour `hubble` ([documentation](https://docs.cilium.io/en/stable/gettingstarted/hubble_setup/#install-the-hubble-client)). Sur votre VM, GitPod ou CodeSpaces, l'outil est **déjà pré-installé** :

```shell
# hubble version
hubble 0.15.0 compiled with go1.23.0 on linux/amd64
```
...
**Note** : Hubble installé via Helm est recommandé plutôt que via CLI séparé.
Vérifiez que le paragraphe suivant est présent dans le fichier ebpf-values.yaml utilisé dans la commande Helm:
```yaml
hubble:
  enabled: true
  relay:
...
Hubble UI est inclus avec Cilium 1.16+. Vous pouvez la lancer via :
```bash
cilium hubble ui
```

L'UI écoutera sur le port TCP/12000 (ou accédez-la via port-forward) :
```
...
**Recommandé** : pour un accès plus robuste, utilisez port-forward :
```shell
...
```shell
# Observer en continu avec plus de détails
kubectl exec -it ds/cilium -n kube-system -c cilium-agent -- hubble observe -f --output json

# Filtrer par namespace
kubectl exec -it ds/cilium -n kube-system -c cilium-agent -- hubble observe -n default

# Voir seulement les connexions bloquées
kubectl exec -it ds/cilium -n kube-system -c cilium-agent -- hubble observe --verdict DROPPED
```

Pour une liste complète des options, utilisez :
```shell
kubectl exec -it ds/cilium -n kube-system -c cilium-agent -- hubble observe --help
```

## Hubble Relay

Pour avoir des informations aggrégées de tous les noeuds, il faut s'adresser au Hubble Relay qui collecte les informations de tous les serveurs Hubble embarqués dans les cilium-agent.

```shell
kubectl -n kube-system port-forward service/hubble-relay --address 0.0.0.0 --address :: 4245:80
```

Depuis votre VM, vous pouvez interroger Hubble Relay :
```shell
hubble status
```
...
**Filtres utiles** : 
- `-n default` : filtrer par namespace
- `--verdict DROPPED` : voir seulement les flux bloqués
- `--verdict ALLOWED` : voir seulement les flux autorisés
- `--protocol tcp` : filtrer par protocole
- `--pod <namespace>/<pod-name>` : filtrer par pod spécifique

Consultez aussi la [Hubble Cheatsheet](/docs/Isovalent%20-%20Cilium%20Hubble%20Cheat%20Sheet.pdf) pour d'autres options avancées.
...
Depuis [la version 1.15.1](https://isovalent.com/blog/post/cilium-1-15/#correlate-traffic-flows-to-a-network-policy), le champ `ingress_allowed_by` a été ajouté et amélioré dans [la version 1.16](https://isovalent.com/blog/post/cilium-1-16/) :
![](../img/jsonlog.png)
