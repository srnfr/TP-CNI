...
Un admin sophistiqué aurait directemnt executé :
```shell
...
```
Dans l'autre fenêtre de console, vérifier que `redis-leader` n'est plus accessible depuis le Pod `debug-blue` crée dans le TP précedent
...
```
Cette commande fonctionnait bien avec cilium-cli en v1.12, mais une erreur (`Error: Failed to decode nested JSON: invalid character 'M' looking for beginning of object key string`) est générée en v1.14 (cf [reportbug](https://github.com/cilium/cilium/issues/29247)).

Le coutournement consiste à selectionner un output yaml (`-o yaml`)
...
D'autres commandes permettent d'avoir une vue d'ensemble des Endpoints
```shell
kubectl get cep
```
