...
```shell
kubectl create deployment hello-world --image=stefanprodan/podinfo:4.0.3 --replicas=5
...
Il existe même une autre façon dans le cas d'un DaemonSet, plus simple (pas besoin de connaître le nom du Pod), mais sans pouvoir influer sur le choix du Pod :
...
De retour dans notre shell interactif GitPod, connectons-nous dans un Pod Cilium pour lister les services et les endpoints :
```shell
...
Analysez le résultat, vous devriez identifier ce genre de lignes:
```
...
Pour la petit histoire, si certains s'intéressent au paramètre `probability` des commands `iptables`, en voici [l'explication](https://github.com/kubernetes/kubernetes/issues/37932)
## Cleanup
