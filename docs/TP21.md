...
## Etape 7 Optimiser le routage [bonus]

Utiliser les [Service Affinity](https://docs.cilium.io/en/stable/network/clustermesh/affinity/) pour biaiser le routage au plus des nodes du cluster d'entrée.

Autre possibilité, utiliser l'attribut K8S `PreferSameZone`

## Etape 8 Chiffrement des flux inter-Nodes [bonus]

Dans le cas actuel, les flux passent clair.

Chiffrez-les en WireGuard en ajoutant les valeurs suivant au values.yaml utilisé par Helm
```yaml
encryption
  enabled=true
  type=wireguard
