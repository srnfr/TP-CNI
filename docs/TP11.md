...
## But du TP
Comprendre comment les @IP exterieures sont modifiées (i.e SNAT) au niveau des Pod serveur.
Montrer comment eBPF permet de conserver l'@IP source.
...
Depuis Gitpod (puisque votre PC est probablement filtré) faites des requetes sur ce port sur les différentes et observez la page service.
Vous devriez obtenir quelque chose comme ceci :
```
% curl 165.227.157.62:30107
{
        "service": {
                "namespace": "default",
                "name": "frontend"
